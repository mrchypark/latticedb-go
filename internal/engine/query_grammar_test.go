...
const auditedCypherParserDigest = "901a81aa7f8b3b3f0c91863f59a59823ab5a4edbe6531b45d910d80f65d3d3cb"
...
func TestQueryGrammarMatrix(t *testing.T) {
	accepted := map[string]string{
		...
		"quoted asterisks in edge identifiers": "MATCH (a)-[`r*x`:`a*b`]->(b) RETURN `r*x`",
		"quoted asterisks with variable hops":  "MATCH (a)-[`r*x`:`a*b`*1..3]->(b) RETURN `r*x`",
	...
	rejected := map[string]string{
		...
		"duplicate variable-hop delimiter": `MATCH (a)-[r:LINK*1..2*3]->(b) RETURN r`,
		"compound top level create":        `CREATE (a)-[:KNOWS]->(b)`,
		"unknown function":                 `MATCH (n:Person) RETURN nope(n.name) AS value`,
		"function arity too few":           `MATCH (n:Person) RETURN toLower() AS lowered`,
		"function arity too many":          `MATCH (n:Person) RETURN size(n.name, n.name) AS width`,
		"order by computed projection":     `MATCH (n:Person) RETURN toLower(n.name) AS lowered ORDER BY lowered`,
